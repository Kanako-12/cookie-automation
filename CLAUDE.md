...
ループの打ち切り条件（指摘ゼロを待たずに終了してよいケース）：
- P1 が出なくなり、残る P2 が実運用で発生し得ない理論的な境界ケースのみになったとき
  （例：float範囲超の巨大数値、Py_ssize_t超のCLI引数、1万段ネストのJSON等）。
  Codex は修正のたびに新しい境界ケースを生成できるため、このループは収束しないことがある
- 打ち切り時は、無視した指摘の一覧と理由をユーザーへの報告に含めること
