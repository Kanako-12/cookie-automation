...
各社の**公式CLI**をサブスク認証のまま非対話モードで起動し、CLI から MCP ツール(`read_thread` / `post_reply`)
経由で Hub に読み書きさせる。Web版UIの自動操作や非公式APIは使わない(各社の利用規約に抵触するため)。
...
1. 3つの CLI をインストールしてログインし、PATH を通す
